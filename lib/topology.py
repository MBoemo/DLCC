class Topology:
	...
	def fun_nestedLogicDigraph_2_topologyDigraph(self,G_nestedLogicDigraph,str_time):
		...
		G_topology_Digraph = self.fun_refineTopology(G_topology_Digraph,str_time) # refine topology - may want to make this an option
		...
		for path in lst_paths:
			...
			if len(lst_orderedNodes) >= 3: # we need a path of at least length 3 to be able to refine topology 
				for i in range(len(lst_orderedNodes) - 2):
					if lst_orderedNodes[i][0] == '1' and lst_orderedNodes[i + 1][0] == '1': # if you've found a series of 1 blocking 1 blocking something else...
						if len(G_topology_refined.predecessors(lst_orderedNodes[i])) == 1 and len(G_topology_refined.predecessors(lst_orderedNodes[i+1])) == 1:
