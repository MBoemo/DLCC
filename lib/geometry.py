class Track_Layout:
	...
	def plot_tracks(self,G,node,starting_coords,direction,track_lengths):
		...
		x = [x + int(starting_coords[0]) for x in range(int_length)]
		y = [int(starting_coords[1])]*int_length
		area = np.pi*(100)
		if direction == -1:
		        self.plt.scatter(y,x,s=area,alpha=0.5)
		        self.plt.arrow(y[-1],x[-1],0,-int_length+1,head_width=0.2,head_length=0.2,fc='g',ec='g',length_includes_head=True)
		        self.plt.text(y[-1],x[-1],node,size='xx-large')
		else:
		        self.plt.scatter(x,y,s=area,alpha=0.5)
		        self.plt.arrow(x[-1],y[-1],-int_length+1,0,head_width=0.2,head_length=0.2,fc='g',ec='g',length_includes_head=True)
